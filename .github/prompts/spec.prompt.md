---
agent: 'agent'
description: Generate a specification for AI-assisted implementation
---

Generate a specification for AI-assisted implementation of the following feature:

**Feature:** ${input:feature:Describe the feature to spec out}

## Instructions

The project conventions are already defined in `.github/copilot-instructions.md` and `.github/instructions/*.instructions.md` — follow them strictly. Ask only if blocked on something not covered there.

Create `.ai/specs/<feature-slug>.md` using the template below. The slug must be lowercase kebab-case derived from the feature name.
...
1. Check `.ai/specs/` for the highest existing number, then increment by 1 (start at `001` if empty)
2. Spec saved to `.ai/specs/<NNN>-<feature-slug>.md` (create `.ai/specs/` if it doesn't exist)
3. Review for completeness — could a new agent implement T1 with no other context?
4. To implement a task: open a new chat → use the `task` prompt → reference the spec file
